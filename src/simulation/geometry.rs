...
impl Tri {
    ...
    pub fn intersect_sphere(&self, b: &Sphere) -> bool {
        ...
        let dist = if (0. ..=1.).contains(&u) && (0. ..=1.).contains(&v) && (0. ..=1.).contains(&w) {
            ...
            let mut res = b.radius + 1.;
            res = res.min(distance_between(self.p[0], e1, b.center));
            res = res.min(distance_between(self.p[1], e2, b.center));
            res = res.min(distance_between(self.p[2], e3, b.center));
            res
        };
